...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    await persistAutomationRun({
      ...
      detail: degraded ? scan.warnings.slice(0, 3).join(" | ") || "One or more market-data feeds were degraded." : "Daily autonomous settle → train → scan cycle completed.",
    });

    console.log("TradesWithMe V4.1 autonomous learning cycle", JSON.stringify({
      at: scan.at,
    ...
    console.error("TradesWithMe V4.1.1 autonomous cycle failed", detail);
    await persistAutomationRun({
      status: "FAILED",
      actionableSettled,
      shadowSettled,
