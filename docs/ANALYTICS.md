# TradesWithMe V4.1.1 — Analytical design

V4.1.1 is designed to make weak evidence visible rather than hide it behind an AI confidence badge. It does not claim that a signal is profitable until the signal survives both historical and forward checks.
...
A single train/test split can flatter a strategy by chance. V4.1.1 therefore adds rolling forward windows. For every strategy it records:
...
V4.1.1 calculates trailing return correlation between symbols of the same asset class. A proposed trade can be downgraded when it is highly correlated with a holding you already own. The overnight paper allocator also refuses to stack highly correlated candidates together above the configured threshold.
...
This is not a complete news feed. V4.1.1 deliberately labels unverified event risk so that the user knows a current-news check is still required.
...
If both stop and target are touched inside the same daily candle, V4.1.1 scores the outcome as a stop because daily OHLC data cannot prove which happened first. This intentionally biases the validation ledger conservatively.
...
The trust score is sample-size shrunk. It cannot receive a robust label with a tiny sample, even if the first few trades happen to win. V4.1.1 currently expects at least 30 completed signals before treating the evidence as usable and 100 before a `ROBUST` label is possible.
...
## V4.1.1 adaptive-learning overlay

V4.1.1 adds a bounded learner on top of the existing evidence engine. Only completed forward signals are eligible for training. The original agent votes are preserved in the ledger, so the learner can measure whether each strategy agent was directionally useful after the signal was frozen.
