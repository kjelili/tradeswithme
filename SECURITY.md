# TradesWithMe V4.1.4 security notes

TradesWithMe V4.1.4 is intended for one owner's private research workflow.

- Keep `APP_ACCESS_CODE`, `CRON_SECRET`, `SUPABASE_SECRET_KEY` and `ALPHAVANTAGE_API_KEY` only in server-side Vercel environment variables.
...
- V4.1.4 does not contain a live broker execution adapter. The overnight allocator is paper-only.
