  ...
  return (
    <main className="shell">
      <header className="topbar">
        <div className="brand-wrap"><div className="logo-mark"><span>T</span></div><div><div className="brand">TRADES<span>WITHME</span></div><div className="tagline">AUTONOMOUS LEARNING · V4.1.1</div></div></div>
      ...
      <section className="panel health-panel">
        <div className="panel-head"><div><span className="section-kicker">DATA INTEGRITY</span><h2>Market data health</h2></div><span className="mini-tag">V4.1.1 FAIL-CLOSED</span></div>
      ...
      <section className="panel lab-panel">
        ...
        {!lab ? <div className="empty-state compact">Choose a symbol above and run Strategy Lab. V4.1.1 checks the held-out tail and multiple rolling forward windows.</div> : <StrategyLab lab={lab} />}
      </section>
...
function ObservatoryDesk({ observatory }: { observatory: LearningObservatory }) {
  if (!observatory.configured) return <div className="validation-warning">Supabase is not configured. Run the V4.1.1 schema so autonomous runs, pending observations and training history can persist.</div>;
    ...
    <div className="observatory-grid">
      <div>
        ...
        {!observatory.pending.length ? <p className="muted-copy">No frozen observations are waiting to mature yet. The next qualifying autonomous scan can add them.</p> : <div className="maturity-table">
          <div className="maturity-head"><span>Type</span><span>Symbol</span><span>Bars left</span><span>Est. maturity</span></div>
          {observatory.pending.slice(0, 12).map((o) => <div className="maturity-row" key={`${o.kind}-${o.id}`}><span>{o.kind === "ACTIONABLE" ? "LIVE TEST" : "SHADOW"}</span><b>{o.symbol}</b><span>{o.estimatedBarsRemaining}/{o.horizonBars}</span><span>{new Date(o.estimatedMaturityAt).toLocaleDateString()}</span></div>)}
        </div>}
      </div>
      <div>
        <div className="subhead">RECENT AUTONOMOUS RUNS</div>
        {!observatory.recentRuns.length ? <p className="muted-copy">The Vercel cron has not logged a V4.1.1 cycle yet. The first run is scheduled automatically.</p> : <div className="automation-run-list">{observatory.recentRuns.slice(0, 6).map((run) => <div className="automation-run" key={run.id}><div><b className={run.status === "FAILED" ? "down" : "up"}>{run.status}</b><small>{new Date(run.ranAt).toLocaleString()}</small>{run.detail ? <small className={run.status === "FAILED" ? "down" : ""}>{run.detail}</small> : null}</div><span>settled {run.actionableSettled + run.shadowSettled}</span><span>frozen {run.actionableInserted + run.shadowInserted}</span><span>{run.learningMode ?? "—"} · {run.effectiveSamples ?? 0} eff.</span></div>)}</div>}
      </div>
    </div>
...
function ValidationDesk({ validation }: { validation: ValidationSummary }) {
  if (!validation.configured) return <div className="validation-body"><div className="validation-warning">Supabase is not configured. Run the V4.1.1 schema migration and add the two Supabase environment variables so signals can be frozen and scored over time.</div>{validation.notes.map((n) => <p className="validation-note" key={n}>• {n}</p>)}</div>;
