# V4.1.4 Autonomous Learning Observatory
...
The order matters: V4 previously trained after scoring the current scan, which meant fresh evidence could wait another cycle before influencing analysis. V4.1.4 trains first.
...
## V4.1.4 maturity semantics

The Observatory no longer treats calendar days as completed market bars. Pending progress uses the latest verified `asOf` date for the specific symbol from the latest persisted scan. Settlement itself remains stricter: it only completes an observation when the market-data history contains the required number of bars strictly after the frozen entry date.

Per-symbol settlement failures are surfaced as diagnostics instead of being silently converted into `settled: 0`.
