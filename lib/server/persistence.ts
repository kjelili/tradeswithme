...
export async function persistScan(scan: ScanResult) {
  ...
  const response = await supabaseFetch(`${config.url}/rest/v1/twm_scans`, {
    method: "POST",
...
export async function loadPersistedScans(limit = 12): Promise<{ configured: boolean; scans: ScanResult[] }> {
  ...
  const response = await supabaseFetch(`${config.url}/rest/v1/twm_scans?select=payload&order=scanned_at.desc&limit=${safeLimit}`, {
...
export async function persistOwnerState(state: Omit<OwnerState, "updatedAt">) {
  ...
  const response = await supabaseFetch(`${config.url}/rest/v1/twm_owner_state?on_conflict=id`, {
    method: "POST",
...
export async function loadOwnerState(): Promise<{ configured: boolean; state: OwnerState | null }> {
  ...
  const response = await supabaseFetch(`${config.url}/rest/v1/twm_owner_state?select=symbols,positions,settings,updated_at&id=eq.owner&limit=1`, {
    headers: headers(config),
    cache: "no-store",
  });
