...
async function loadTableRows(config: { url: string; key: string }, table: "twm_forward_signals" | "twm_shadow_signals", kind: LearningRow["kind"], weight: number, limit: number) {
  const response = await fetch(`${config.url}/rest/v1/${table}?select=id,asset_class,opened_at,return_pct,outcome,confidence,agent_snapshot&outcome=neq.PENDING&return_pct=not.is.null&order=opened_at.asc&limit=${limit}`, {
...
async function loadStoredPolicy(): Promise<LearningPolicy | null> {
  ...
  const response = await fetch(`${config.url}/rest/v1/twm_learning_state?select=payload&id=eq.owner&limit=1`, { headers: headers(config), cache: "no-store" });
...
async function persistPolicy(policy: LearningPolicy) {
  ...
  const state = await fetch(`${config.url}/rest/v1/twm_learning_state?on_conflict=id`, {
    method: "POST",
  ...
  const log = await fetch(`${config.url}/rest/v1/twm_learning_runs`, {
